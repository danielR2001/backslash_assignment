...
export const bodyValidation = (schema: Joi.Schema) => {
  return (req: Request, res: Response, next: NextFunction) => {
    const { error } = schema.validate(req.body, { errors: { label: 'key', wrap: { label: false } } })
    ...
    logger(Level.Debug, 'DEBUG', `body validation failed. details: ${JSON.stringify(details)}`)

    next(new BadRequestError(InvalidParamsMessage.INVALID_PARAMS, invalidParamsMessage))
  }
}
